...
func deploySiteAction(cfg map[string]any) (*Response, error) {
	...
	// 2. 解析传入域名
	userDomains, isMultiple := core.ParseDomainsFixedSeparator(xpDomain, ",")
	if isMultiple {
	...
	// 创建请求客户端
	openapiClient, err := openapi.NewClient(xpURL, xpApiKey)
	if err != nil {

...
import (
	...
	"github.com/dtapps/allinssl_plugins/open_resty_manager/certificate"
	"github.com/dtapps/allinssl_plugins/open_resty_manager/openapi"
	"github.com/dtapps/allinssl_plugins/open_resty_manager/site"
)
...
func deploySiteAction(cfg map[string]any) (*core.Response, error) {
	...
	// 2. 解析传入域名
	userDomains, isMultiple := core.ParseDomainsFixedSeparator(omDomain, ",")
	if isMultiple {
	...
	for _, domain := range userDomains {
		err = site.Action(openapiClient, domain, certBundle)
		if err != nil {
			return nil, err
		}
	}
